...
```bash
export PROJECT_ID=<the-gcp-project>   # same project as rig-ads-website
...
gcloud run deploy rig-directory \
  ...
  --memory=512Mi --cpu=1 --min-instances=1 --allow-unauthenticated
```
...
```bash
# service account the scheduler uses to call Cloud Build
