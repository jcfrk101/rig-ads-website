...
const PHONE_NUMBER_DISPLAY = '1-855-744-2223'
const PHONE_NUMBER_TEL = 'tel:18557442223'
...
const CallButton: FunctionComponent<{ size?: 'large' | 'hero'; isMobile?: boolean }> = ({
  size = 'large',
  isMobile = false,
}) => {
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/admin/a52d35fa-b696-4a13-93e6-a31f4f98d9a7/service/metrics`)
      ...
      .catch(() => {
        // silently fail — metrics section stays hidden
      })
  }, [])

  // Use live API values; fall back to last-known real values while fetch is in flight
  ...
  return (
    <>
      <Head>
        <title>RIG — Truck Repair &amp; Roadside Assistance | Call 1-855-744-2223</title>
        <meta
          name="description"
          content="Truck broke down? RIG connects you to 6,000+ mechanics nationwide. Get multiple bids within minutes. Call 1-855-744-2223 — available 24/7."
        />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta property="og:title" content="RIG — Truck Repair &amp; Roadside Assistance" />
        <meta
          property="og:description"
          content="Get roadside help fast. 6,000+ mechanics, multiple bids, real-time tracking. Call 1-855-744-2223."
        />
        ...
        <Box style={{ backgroundColor: '#323E48', width: '100%' }}>
          ...
          >
            <img
              ...
              alt="RIG Logo"
              style={{ width: isMobile ? '60px' : '80px', height: isMobile ? '45px' : '60px' }}
            />
            <Box
              component="a"
              href={PHONE_NUMBER_TEL}
            ...
            >
              ...
              {isMobile ? 'Call Now' : PHONE_NUMBER_DISPLAY}
            </Box>
          </Box>
          ...
          >
            ...
            <Box style={{ display: 'flex', justifyContent: 'center', marginBottom: '16px' }}>
              <CallButton size="hero" isMobile={isMobile} />
            </Box>
            <Box
              style={{
                display: 'flex',
            ...
            >
              ...
              {!isMobile && (
                <Typography style={{ color: 'rgba(255,255,255,0.15)', fontSize: '0.8rem' }}>·</Typography>
              )}
              <Typography style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.8rem', letterSpacing: '0.04em' }}>
                {trucksServiced} trucks serviced since {servicingSince}
              </Typography>
            </Box>
          </Box>
          ...
          {(
            <Box style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              {/* Three metric panels (no "Quotes Per Job") */}
              <Box
                style={{
                  display: 'flex',
                  flexDirection: isMobile ? 'column' : 'row',
              ...
              {[
                {
                  ...
                  title: '6,000+ Mechanics Nationwide',
                  desc: 'Access our network of verified mechanics and shops across every major route in the US.',
                },
                {
                  icon: '/static/icons/currency-filled.svg',
                  title: 'Competitive Pricing',
                  desc: `RIG customers pay ${Math.floor(((INDUSTRY_AVG_COST - rigAvgCost) / INDUSTRY_AVG_COST) * 100)}% less for road service than the industry average. Same repair, lower cost.`,
                },
                {
                  icon: '/static/icons/clock.svg',
                  title: 'Fast Response Times',
                  desc: 'Receive your first offer in minutes, not hours. Get back on the road sooner.',
                },
                {
        ...
        >
          <Box style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <img
              src="/static/icons/logo.svg"
              alt="RIG Logo"
              style={{ width: '40px', height: '30px', opacity: 0.7 }}
            />
